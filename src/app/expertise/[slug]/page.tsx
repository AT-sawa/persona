...
import { getCategoryBySlug } from "@/lib/case-categories";
...
export default async function ExpertiseDetailPage({ params }: Props) {
  ...
  // Get matching category for linking to case archive pages
  const matchingCategory = getCategoryBySlug(slug);
          ...
          <section className="py-20 px-6 bg-[#f2f2f7]">
            <div className="max-w-[1000px] mx-auto">
              <div className="text-center mb-12">
                ...
                <p className="text-[13px] text-[#888] mt-3">
                  {area.name}領域の案件を業界別にご覧いただけます
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {INDUSTRY_SLUGS.map((indSlug) => {
                  const ind = INDUSTRY_AREAS[indSlug];
                  return (
                    <Link
                      key={indSlug}
                    ...
                    >
                      ...
                      <h3 className="text-[15px] font-bold text-[#091747] leading-[1.5] mb-2 group-hover:text-[#1FABE9] transition-colors">
                        {ind.name}の{matchingCategory.name}案件
                      </h3>
                      <p className="text-[12px] text-[#888] leading-[1.7] line-clamp-2 mb-3">
                        {ind.name}業界×{matchingCategory.name}のフリーコンサル案件一覧
                      </p>
